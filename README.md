...
Created by Thomas Gumricht, Stockholm University for the EU funded project [Ai4SoilHealth](https://ai4soilhealth.eu).

Last edits: 26 August 2025

## Objective

The Jupyter notebook  [AI4SH_IN-SITU_DATA_MANAGEMENT]() converts in-situ data from soil sampling camapaigns into organised json documents. The input data must be organised as a Comma Separated Values (CSV) file, typically created by first putting the in-situ sampled data together in a spreadsheet and then exporting the spreadsheet as a .csv file.

## Input data requirements and organisation

To run the notebook at least four files must be defined, one defining the user project settings (_user project file_), one defining process parameters (_process file_), one with the actual data (_data source file_), and one that defines the columns (header) in the data file (_method source file_). The latter two are defined as process parameters in the _process file_, whereas the _user project file_ and the _process file_ are defined directly in the notebook.  

### User project file

The user project file must be in json format and defines the path to the root folder where all the other files can be found. The user project file also defines the user and if database access is required (not in the present version of the notebook) the login credentials for the given also user must be given.
...
A _process_file_, whether linked directly in the notebook or via a _job file_ must include the _sub_process_id_ (at present the only defined _sun_process_id_ is _import_csv_single-lines_) and the full paths to the _data source file_ (_data_src_FPN_), the _method source file_ (_method_src_FPN_) and the path to the destination directory (_dst_FP_) where to save the json formatted output. 

If there are metadata entries that are exactly identical for all records (lines) in the _data source file_ (e.g. the person responsible for the field sampling, the analysis or the sample logistics), then this metadata can be stated in the _process file_ instead of being repeated for evay record (line) in the _data source file_. 
...
In a _job file_ you can link to a sequence of _process files_ either as an array directly in the _job file_ or by lining to a _pilot file_ (simple text file) that lists the _process files_ to execute.
...
The json file objects _job_folder_ (_doc_ in the example above) and _process_sub_folder_ (_json_) in the example above, refer to the hierarchical path structure of the fiels to process. For the example above the strucure that is expected is this:
...
that then lists the _process_files_ to run (blank lines or lines starting with hashtag are ignored):
...
Required metadata to generate the json output are:
- pilot_country,
- pilot_site,
- sample_id,
- min_depth,
